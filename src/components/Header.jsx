...
export const Header = () => {
  ...
  // دالة يتم تنفيذها عند إرسال البحث
  const handleSearchSubmit = (e) => {
    e.preventDefault(); // منع إعادة تحميل الصفحة
    if (searchQuery.trim()) { // تأكد من أن البحث ليس فارغًا
      navigate(`/search?q=${searchQuery}`); // انتقل إلى صفحة نتائج البحث
      setSearchQuery(''); // أفرغ حقل البحث
    }
  };
      ...
      <div className="container mx-auto p-4 flex justify-between items-center">
        ...
        <form onSubmit={handleSearchSubmit} className="hidden md:flex items-center">
          <input 
            ...
            className="border rounded-l-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
